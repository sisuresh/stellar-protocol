...
```
CAP: 0080
Title: Host functions for efficient ZK implementations
Working Group:
...
BN254 MSM (Multi-Scalar Multiplication) and modular arithmetic are used for a variety of ZK use cases. Adding host support for these will greatly improve the performance of these use cases. 
...
Stellar has host support for the BN254 functions available on the EVM (G1 Add, G1 Mul, and Pairing), but there are use cases that require many G1 add and multiplies, as well as lots of modular arithmetic. These use cases end up being expensive due to all the work done on the guest. By adding a BN254 MSM host function, as well as BN254 host functions for modular arithmetic (add, sub, mul, pow, and inv), we'll enable more interesting BN254 use cases on Stellar.
...
#### Field and groups
See [CAP-0074](./cap-0074.md#field-and-groups) for definitions of the fields and groups. 
...
**Cost**: conversion of fr from U256 (`Bn254FrFromU256`), scalar addition `Bn254FrAddSub`, and conversion back to U256.
...
**Cost**: conversion of fr from U256 (`Bn254FrFromU256`), scalar subtraction `Bn254FrAddSub`, and conversion back to U256.
...
**Cost**: conversion of fr from U256 (`Bn254FrFromU256`), scalar multiplication `Bn254FrMul`, and conversion back to U256.
...
**Cost**: conversion of fr from U256 (`Bn254FrFromU256`), scalar exponentiation `Bn254FrPow`, and conversion back to U256.
...
**Cost**: conversion of fr from U256 (`Bn254FrFromU256`), scalar inversion `Bn254FrInv`, and conversion back to U256.
